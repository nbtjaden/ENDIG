...
This script cleans the 2018 ECDC surveillance system data, turning the xlsx file into a collection of sf-compatible objects.
...
```{r}
disease_IDs <- meta %>% select(ID_master, ID_ECDC_2018)
disease_IDs
```
...
Note that these are 31 instead of the expected 27 countries. The list additionally includes: Iceland, Liechtenstein, Norway, United Kingdom. As of 2018, the UK is still on the list for Creutzfeldt-Jacob Disease (VCJD), but does not report anything else to ECDC.
...
```{r}
survtype_2018_compulsory <- data.frame(Country=countries)
survtype_2018_comprehensive <- data.frame(Country=countries)
survtype_2018_active <- data.frame(Country=countries)
survtype_2018_aggregated <- data.frame(Country=countries)
...
for (i in 1:nrow(disease_IDs)){ # iterate over all diseases
  ...
  # read data
  if(!is.na(dis[2])){
    # If there is a sheet for this disease in the 2018 workbook,
    # read the data for this disease:
    dat <-  readxl::read_xlsx(path = file.path(getwd(), "data/raw/Table-surveillance_systems_overview_2018.xlsx"),
                              skip = 2,
                              col_types = "text",
  ...
  if(nrow(dat)==0){ 
    ...
    filler <- data.frame(rep("no data", times=nrow(survtype_2018_compulsory)))
    names(filler) <- dis[1]
    survtype_2018_compulsory <- cbind(survtype_2018_compulsory, filler)
    survtype_2018_comprehensive <- cbind(survtype_2018_comprehensive, filler)
    survtype_2018_active <- cbind(survtype_2018_active, filler)
    survtype_2018_aggregated <- cbind(survtype_2018_aggregated, filler)
  }else{
    ...
    survtype_2018_compulsory <- cbind(survtype_2018_compulsory, dat2[,2])
    
    dat2 <- dat %>% select(Country, compre)  %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2018_comprehensive <- cbind(survtype_2018_comprehensive, dat2[,2])
    
    dat2 <- dat %>% select(Country, act) %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2018_active <- cbind(survtype_2018_active, dat2[,2])
    
    dat2 <- dat %>% select(Country, agg) %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2018_aggregated <- cbind(survtype_2018_aggregated, dat2[,2])
  }
}
```
...
```{r}
survtype_2018_compulsory <- survtype_2018_compulsory %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2018_comprehensive <- survtype_2018_comprehensive %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2018_active <- survtype_2018_active %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2018_aggregated <- survtype_2018_aggregated %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
```
...
```{r}
saveRDS(survtype_2018_compulsory, file=file.path(getwd(), "data/processed/survtype_2018_compulsory.rds"))
saveRDS(survtype_2018_comprehensive, file=file.path(getwd(), "data/processed/survtype_2018_comprehensive.rds"))
saveRDS(survtype_2018_active, file=file.path(getwd(), "data/processed/survtype_2018_active.rds"))
saveRDS(survtype_2018_aggregated, file=file.path(getwd(), "data/processed/survtype_2018_aggregated.rds"))
```
...
```{r}
countries_2018_compulsory <- countries_sf %>% left_join(survtype_2018_compulsory, by=c("name_long" = "Country"))
countries_2018_comprehensive <- countries_sf %>% left_join(survtype_2018_comprehensive, by=c("name_long" = "Country"))
countries_2018_active <- countries_sf %>% left_join(survtype_2018_active, by=c("name_long" = "Country"))
countries_2018_aggregated <- countries_sf %>% left_join(survtype_2018_aggregated, by=c("name_long" = "Country"))
```

Export this data for further use elsewhere.

```{r}
saveRDS(countries_2018_compulsory, file=file.path(getwd(), "data/processed/countries_2018_compulsory.rds"))
saveRDS(countries_2018_comprehensive, file=file.path(getwd(), "data/processed/countries_2018_comprehensive.rds"))
saveRDS(countries_2018_active, file=file.path(getwd(), "data/processed/countries_2018_active.rds"))
saveRDS(countries_2018_aggregated, file=file.path(getwd(), "data/processed/countries_2018_aggregated.rds"))
```
