...
table_heatmap <- data.frame(expand.grid(Year = years, Country = countries))
data_heatmap <- list()
for(dis in meta$ID_master){
  data_heatmap[[dis]] <- table_heatmap
}
...
for(year in years){
  ...
  for (i in 1:nrow(disease_IDs)){
    ...
    data_heatmap[[dis_global]] <- dat_hm %>% left_join(x=table_heatmap, by = c("Year", "Country"))
  }
}
```

## Export data to file system
Exports the prepared data objects for use in the shiny app.
