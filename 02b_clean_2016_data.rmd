...
This script cleans the 2016 ECDC surveillance system data, turning the xlsx file into a collection of sf-compatible objects.
...
```{r}
disease_IDs <- meta %>% select(ID_master, ID_ECDC_2016)
disease_IDs
```
...
Note that these are 31 instead of the expected 27 countries. The list additionally includes: Iceland, Liechtenstein, Norway, United Kingdom. As of 2016, the UK is still on the list for Creutzfeldt-Jacob Disease (VCJD), but does not report anything else to ECDC.
...
### Special treatment notes 2016

The *HIV and AIDS* sheet has a different layout, with two separate tables for HIV and AIDS respectively. *Hep B* and *Hep C* are missing a column. Otherwise see console output below.

### Process Data

```{r}
survtype_2016_compulsory <- data.frame(Country=countries)
survtype_2016_comprehensive <- data.frame(Country=countries)
survtype_2016_active <- data.frame(Country=countries)
survtype_2016_aggregated <- data.frame(Country=countries)
...
for (i in 1:nrow(disease_IDs)){ # iterate over all diseases
  ...
  if(!dis[2] %in% "HIV and AIDS"){
    # read data
    if(!is.na(dis[2])){
      # If there is a sheet for this disease in the 2016 workbook,
      # read the data for this disease:
      if(!dis[2] %in% c("Hep B", "Hep C")){
      dat <-  readxl::read_xlsx(path = file.path(getwd(), "data/raw/Table-surveillance_systems_overview_2016.xlsx"),
                                skip = 5,
                                col_types = "text",
                                col_names = c("Country", "src", "compu", "compre", "act", "agg", "L", "P", "H", "O", "casedef"),
                                sheet = dis[2])
      }else{# HEPB and HEPC have an additional column 
        dat <-  readxl::read_xlsx(path = file.path(getwd(), "data/raw/Table-surveillance_systems_overview_2016.xlsx"),
                                skip = 5,
                                col_types = "text",
                                col_names = c("Country", "src", "compu", "compre", "act", "agg", "L", "P", "H", "O", "casedef", "coverage"),
                                sheet = dis[2])
      }
  ...
  # Special treatment for HIV/AIDS sheet starts here  
    ...
    dat$compre[dat$compre %in% "NS/unk"] <- "."
  ...
  if(nrow(dat)==0){ 
    ...
    filler <- data.frame(rep("no data", times=nrow(survtype_2016_compulsory)))
    names(filler) <- dis[1]
    survtype_2016_compulsory <- cbind(survtype_2016_compulsory, filler)
    survtype_2016_comprehensive <- cbind(survtype_2016_comprehensive, filler)
    survtype_2016_active <- cbind(survtype_2016_active, filler)
    survtype_2016_aggregated <- cbind(survtype_2016_aggregated, filler)
  }else{
    ...
    survtype_2016_compulsory <- cbind(survtype_2016_compulsory, dat2[,2])
    
    dat2 <- dat %>% select(Country, compre)  %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2016_comprehensive <- cbind(survtype_2016_comprehensive, dat2[,2])
    
    dat2 <- dat %>% select(Country, act) %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2016_active <- cbind(survtype_2016_active, dat2[,2])
    
    dat2 <- dat %>% select(Country, agg) %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2016_aggregated <- cbind(survtype_2016_aggregated, dat2[,2])
  }
}
```
...
```{r}
survtype_2016_compulsory <- survtype_2016_compulsory %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2016_comprehensive <- survtype_2016_comprehensive %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2016_active <- survtype_2016_active %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2016_aggregated <- survtype_2016_aggregated %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
```
...
```{r}
saveRDS(survtype_2016_compulsory, file=file.path(getwd(), "data/processed/survtype_2016_compulsory.rds"))
saveRDS(survtype_2016_comprehensive, file=file.path(getwd(), "data/processed/survtype_2016_comprehensive.rds"))
saveRDS(survtype_2016_active, file=file.path(getwd(), "data/processed/survtype_2016_active.rds"))
saveRDS(survtype_2016_aggregated, file=file.path(getwd(), "data/processed/survtype_2016_aggregated.rds"))
```
...
```{r}
countries_2016_compulsory <- countries_sf %>% left_join(survtype_2016_compulsory, by=c("name_long" = "Country"))
countries_2016_comprehensive <- countries_sf %>% left_join(survtype_2016_comprehensive, by=c("name_long" = "Country"))
countries_2016_active <- countries_sf %>% left_join(survtype_2016_active, by=c("name_long" = "Country"))
countries_2016_aggregated <- countries_sf %>% left_join(survtype_2016_aggregated, by=c("name_long" = "Country"))
```

Export this data for further use elsewhere.

```{r}
saveRDS(countries_2016_compulsory, file=file.path(getwd(), "data/processed/countries_2016_compulsory.rds"))
saveRDS(countries_2016_comprehensive, file=file.path(getwd(), "data/processed/countries_2016_comprehensive.rds"))
saveRDS(countries_2016_active, file=file.path(getwd(), "data/processed/countries_2016_active.rds"))
saveRDS(countries_2016_aggregated, file=file.path(getwd(), "data/processed/countries_2016_aggregated.rds"))
```
