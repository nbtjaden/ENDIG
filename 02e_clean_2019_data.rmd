...
This script cleans the 2019 ECDC surveillance system data, turning the xlsx file into a collection of sf-compatible objects.
...
```{r}
disease_IDs <- meta %>% select(ID_master, ID_ECDC_2019)
disease_IDs
```
...
Note that these are 31 instead of the expected 27 countries. The list additionally includes: Iceland, Liechtenstein, Norway, United Kingdom. As of 2019, the UK is still on the list for Creutzfeldt-Jacob Disease (VCJD), but does not report anything else to ECDC.
...
```{r}
survtype_2019_compulsory <- data.frame(Country=countries)
survtype_2019_comprehensive <- data.frame(Country=countries)
survtype_2019_active <- data.frame(Country=countries)
survtype_2019_aggregated <- data.frame(Country=countries)
...
for (i in 1:nrow(disease_IDs)){ # iterate over all diseases
  ...
  # read data
  if(!is.na(dis[2])){
    # If there is a sheet for this disease in the 2019 workbook,
    # read the data for this disease:
    dat <-  readxl::read_xlsx(path = file.path(getwd(), "data/raw/Table-surveillance_systems_overview_2019.xlsx"),
                              skip = 2,
                              col_types = "text",
  ...
  if(nrow(dat)==0){ 
    ...
    filler <- data.frame(rep("no data", times=nrow(survtype_2019_compulsory)))
    names(filler) <- dis[1]
    survtype_2019_compulsory <- cbind(survtype_2019_compulsory, filler)
    survtype_2019_comprehensive <- cbind(survtype_2019_comprehensive, filler)
    survtype_2019_active <- cbind(survtype_2019_active, filler)
    survtype_2019_aggregated <- cbind(survtype_2019_aggregated, filler)
  }else{
    ...
    survtype_2019_compulsory <- cbind(survtype_2019_compulsory, dat2[,2])
    
    dat2 <- dat %>% select(Country, compre)  %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2019_comprehensive <- cbind(survtype_2019_comprehensive, dat2[,2])
    
    dat2 <- dat %>% select(Country, act) %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2019_active <- cbind(survtype_2019_active, dat2[,2])
    
    dat2 <- dat %>% select(Country, agg) %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2019_aggregated <- cbind(survtype_2019_aggregated, dat2[,2])
  }
}
```
...
```{r}
survtype_2019_compulsory <- survtype_2019_compulsory %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2019_comprehensive <- survtype_2019_comprehensive %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2019_active <- survtype_2019_active %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2019_aggregated <- survtype_2019_aggregated %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
```
...
```{r}
saveRDS(survtype_2019_compulsory, file=file.path(getwd(), "data/processed/survtype_2019_compulsory.rds"))
saveRDS(survtype_2019_comprehensive, file=file.path(getwd(), "data/processed/survtype_2019_comprehensive.rds"))
saveRDS(survtype_2019_active, file=file.path(getwd(), "data/processed/survtype_2019_active.rds"))
saveRDS(survtype_2019_aggregated, file=file.path(getwd(), "data/processed/survtype_2019_aggregated.rds"))
```
...
```{r}
countries_2019_compulsory <- countries_sf %>% left_join(survtype_2019_compulsory, by=c("name_long" = "Country"))
countries_2019_comprehensive <- countries_sf %>% left_join(survtype_2019_comprehensive, by=c("name_long" = "Country"))
countries_2019_active <- countries_sf %>% left_join(survtype_2019_active, by=c("name_long" = "Country"))
countries_2019_aggregated <- countries_sf %>% left_join(survtype_2019_aggregated, by=c("name_long" = "Country"))
```

Export this data for further use elsewhere.

```{r}
saveRDS(countries_2019_compulsory, file=file.path(getwd(), "data/processed/countries_2019_compulsory.rds"))
saveRDS(countries_2019_comprehensive, file=file.path(getwd(), "data/processed/countries_2019_comprehensive.rds"))
saveRDS(countries_2019_active, file=file.path(getwd(), "data/processed/countries_2019_active.rds"))
saveRDS(countries_2019_aggregated, file=file.path(getwd(), "data/processed/countries_2019_aggregated.rds"))
```
