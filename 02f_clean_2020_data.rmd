...
This script cleans the 2020 ECDC surveillance system data, turning the xlsx file into a collection of sf-compatible objects.
...
```{r}
disease_IDs <- meta %>% select(ID_master, ID_ECDC_2020)
disease_IDs
```
...
Load previously downloaded country polygons from GISCO:
...
```{r}
survtype_2020_compulsory <- data.frame(Country=countries)
survtype_2020_comprehensive <- data.frame(Country=countries)
survtype_2020_active <- data.frame(Country=countries)
survtype_2020_aggregated <- data.frame(Country=countries)
...
for (i in 1:nrow(disease_IDs)){ # iterate over all diseases
  ...
  # read data
  if(!is.na(dis[2])){
    # If there is a sheet for this disease in the 2020 workbook,
    # read the data for this disease:
    dat <-  readxl::read_xlsx(path = file.path(getwd(), "data/raw/Table-surveillance_systems_overview_2020.xlsx"),
                              skip = 2,
                              col_types = "text",
  ...
  if(nrow(dat)==0){ 
    ...
    filler <- data.frame(rep("no data", times=nrow(survtype_2020_compulsory)))
    names(filler) <- dis[1]
    survtype_2020_compulsory <- cbind(survtype_2020_compulsory, filler)
    survtype_2020_comprehensive <- cbind(survtype_2020_comprehensive, filler)
    survtype_2020_active <- cbind(survtype_2020_active, filler)
    survtype_2020_aggregated <- cbind(survtype_2020_aggregated, filler)
  }else{
    ...
    survtype_2020_compulsory <- cbind(survtype_2020_compulsory, dat2[,2])
    
    dat2 <- dat %>% select(Country, compre)  %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2020_comprehensive <- cbind(survtype_2020_comprehensive, dat2[,2])
    
    dat2 <- dat %>% select(Country, act) %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2020_active <- cbind(survtype_2020_active, dat2[,2])
    
    dat2 <- dat %>% select(Country, agg) %>%
      right_join(data.frame(Country=countries), by="Country")
    names(dat2)[2] <- dis[1]
    dat2 <- arrange(dat2, Country)
    survtype_2020_aggregated <- cbind(survtype_2020_aggregated, dat2[,2])
  }
}
```
...
```{r}
survtype_2020_compulsory <- survtype_2020_compulsory %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2020_comprehensive <- survtype_2020_comprehensive %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2020_active <- survtype_2020_active %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
survtype_2020_aggregated <- survtype_2020_aggregated %>%
  mutate_all(list(~ tidyr::replace_na(., "none"))) 
```
...
```{r}
saveRDS(survtype_2020_compulsory, file=file.path(getwd(), "data/processed/survtype_2020_compulsory.rds"))
saveRDS(survtype_2020_comprehensive, file=file.path(getwd(), "data/processed/survtype_2020_comprehensive.rds"))
saveRDS(survtype_2020_active, file=file.path(getwd(), "data/processed/survtype_2020_active.rds"))
saveRDS(survtype_2020_aggregated, file=file.path(getwd(), "data/processed/survtype_2020_aggregated.rds"))
```
...
```{r}
countries_2020_compulsory <- countries_sf %>% left_join(survtype_2020_compulsory, by=c("name_long" = "Country"))
countries_2020_comprehensive <- countries_sf %>% left_join(survtype_2020_comprehensive, by=c("name_long" = "Country"))
countries_2020_active <- countries_sf %>% left_join(survtype_2020_active, by=c("name_long" = "Country"))
countries_2020_aggregated <- countries_sf %>% left_join(survtype_2020_aggregated, by=c("name_long" = "Country"))
```

Export this data for further use elsewhere.

```{r}
saveRDS(countries_2020_compulsory, file=file.path(getwd(), "data/processed/countries_2020_compulsory.rds"))
saveRDS(countries_2020_comprehensive, file=file.path(getwd(), "data/processed/countries_2020_comprehensive.rds"))
saveRDS(countries_2020_active, file=file.path(getwd(), "data/processed/countries_2020_active.rds"))
saveRDS(countries_2020_aggregated, file=file.path(getwd(), "data/processed/countries_2020_aggregated.rds"))
```
